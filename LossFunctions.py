...
# def confusion(output, target):
#     output[output > 0] = 1
#     target_tensor = torch.tensor(target, dtype=torch.float32)  # Konversi target menjadi Tensor
#     p = torch.sum(target_tensor == 1).item()
#     n = torch.sum(target_tensor == 0).item()
    
#     tp = (output * target_tensor).sum().item()
#     tn = ((1 - output) * (1 - target_tensor)).sum().item()
#     fp = ((1 - target_tensor) * output).sum().item()
#     fn = ((1 - output) * target_tensor).sum().item()
              
#     res = {"P": p, "N": n, "TP": tp, "TN": tn, "FP": fp, "FN": fn, "TPR": (tp / p), "TNR": (tn / n),
